...
export const cleanCommand = async (options?: { force?: boolean; all?: boolean }) => {
  ...
  if (options?.all) {
    if (!options?.force) {
      ...
      console.log(`This will delete Meta-code-graph-base indexes for ${entries.length} repo(s):`);
  ...
  if (!options?.force) {
    console.log(`This will delete the Meta-code-graph-base index for: ${repoName}`);
