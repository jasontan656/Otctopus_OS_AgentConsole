...
- 规则说明：
  - 对模板类技能，`1. 技能定位` 采用两段写法：
    - `技能本体`：写技能本身是什么、承载什么内容、自己的工具是什么。
    - `规则说明`：像旁白一样写这一章怎么写，要求说清哪些信息必须交代、哪些表达应压缩。
  - `规则说明` 只解释本章使用规则，不重复技能本体；门面必须极简，不得在定位段落长篇铺陈。
  - 若确有必要，可在 `规则说明` 中给出进一步展开入口，但只能做轻量提示，不能把扩展说明正文塞回门面。
...
- 规则说明：
  ...
  - 对模板/治理类技能，双段写法可用于全文，但 `规则说明` 始终只解释写法和边界，不承担运行正文。
