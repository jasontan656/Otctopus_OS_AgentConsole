...
- 当前章鱼OS的顶层文件夹只允许使用以下权威分类目录：
  ...
  - `Foundation_Bundle/`
  - `Deploy_Guide/`
- `Foundation_Bundle/` 作为后端根容器，内部固定区分：
  - 平台能力对象：`Auth/`、`Payload/`、`Persistence/`、`Event_Task/`、`Cache/`、`Session_Context/`、`Policy_Enforcement/`、`Storage_Access/`、`Audit_Observe/`
  - 业务模块容器：`Domain_Modules/`
  - 集成适配容器：`Integration_Adapters/`
  - 基础设施合同容器：`Infra_Contracts/`
...
- `Development_Docs/` 负责承载项目级架构说明、开发索引、当前单一前端产品的 mother_doc/graph/runtime 资产与 repo 级证据入口；它不是独立服务对象目录。
- `Deploy_Guide/` 负责承载部署脚本、环境模板、CI 与门禁；它不是业务代码容器。
- `API_Gateway`、`*_Service`、`*_UI`、`*_DB`、`*_Cache`、`MQ_Broker`、`Octopus_Hub`、`Mother_Doc`、`System_Manifests`、`Capability_Modules`、`Integration_Adapters`、`Infra_Contracts`、`Deploy` 这类旧顶层目录名不再允许继续作为权威目录名存在。
- 每个服务/模块对象根目录至少应承载：
  - `README.md`
...
- 当前前端根的产品级开发文档统一收敛在 repo 根 `Development_Docs/`；例如 `Foundation_Bundle/Auth/Development_Docs/` 表示 `Auth` 这个后端平台能力对象自己的开发文档容器。
- `Foundation_Bundle` 作为后端根，不承载共享 `Development_Docs/`；后端对象自己的开发文档应继续落在各自对象根下的 `Development_Docs/`。
