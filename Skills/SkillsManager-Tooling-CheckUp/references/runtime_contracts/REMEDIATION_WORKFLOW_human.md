...
```json
{
  ...
  "instruction": [
    ...
    "When the issue is only language-specific code style or file-shape, do not remediate it here; hand it off to the relevant language constitution."
  ],
  "workflow": [
    "Read the target skill local contract and execution surface before editing.",
    "Use the techstack-baseline, cli-surface, tooling-boundary, and output-governance directives as needed to confirm replacement gates are satisfied.",
    "Rewrite the minimal correct scope, then validate with the target skill's existing tests and lint commands.",
    "If Python files changed, finish by running Dev-PythonCode-Constitution lint on the concrete target scope."
  ],
  "rules": [
