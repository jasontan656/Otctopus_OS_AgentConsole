...
def write_pack(pack_dir: Path, step: dict[str, str], index: int) -> None:
    ...
    pack_dir.joinpath("evidence_registry.json").write_text(json.dumps({"pack_id": pack_id, "evidence_domains": {"code": [], "tests": [], "runtime": [], "logs": [], "browser": [], "network": [], "assets": [], "operator_notes": []}}, indent=2, ensure_ascii=False) + "\n", encoding="utf-8")
