---
doc_id: functional_analysis_runtask.path.analysis_loop.workflow_index
doc_type: workflow_index_doc
topic: Analysis loop workflow index
reading_chain:
- key: research_baseline
  target: steps/research_baseline/00_RESEARCH_BASELINE_ENTRY.md
  hop: branch
  reason: research_baseline 负责锁定目标意图或目标项目、既有资产与证据入口。
- key: architecture_convergence
  target: steps/architecture_convergence/00_ARCHITECTURE_CONVERGENCE_ENTRY.md
  hop: branch
  reason: architecture_convergence 负责把旧资产与当前约束收敛成单技能多阶段目标形态。
- key: plan
  target: steps/plan/00_PLAN_ENTRY.md
  hop: branch
  reason: plan 负责把目标形态拆成最小切片施工合同。
- key: implementation
  target: steps/implementation/00_IMPLEMENTATION_ENTRY.md
  hop: branch
  reason: implementation 只消费 active slice 并逐回合写回事实证据。
- key: validation
  target: steps/validation/00_VALIDATION_ENTRY.md
  hop: branch
  reason: validation 负责阶段闭合、引用有效性与写回一致性收口。
---

# analysis_loop 阶段索引

## 当前入口的复合阶段
1. [research_baseline]：`steps/research_baseline/00_RESEARCH_BASELINE_ENTRY.md`
2. [architecture_convergence]：`steps/architecture_convergence/00_ARCHITECTURE_CONVERGENCE_ENTRY.md`
3. [plan]：`steps/plan/00_PLAN_ENTRY.md`
4. [implementation]：`steps/implementation/00_IMPLEMENTATION_ENTRY.md`
5. [validation]：`steps/validation/00_VALIDATION_ENTRY.md`

## 下一跳列表
- [research_baseline]：`steps/research_baseline/00_RESEARCH_BASELINE_ENTRY.md`
- [architecture_convergence]：`steps/architecture_convergence/00_ARCHITECTURE_CONVERGENCE_ENTRY.md`
- [plan]：`steps/plan/00_PLAN_ENTRY.md`
- [implementation]：`steps/implementation/00_IMPLEMENTATION_ENTRY.md`
- [validation]：`steps/validation/00_VALIDATION_ENTRY.md`
