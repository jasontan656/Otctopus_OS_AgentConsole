...
from .catalog import SKILL_ROOT
from .types import ReadingChainPayload, ReadingChainSegment
...
def _facade_entries(markdown_path: Path) -> list[dict[str, str]]:
    ...
    for raw_line in body.splitlines():
        ...
        if match:
            current = {"key": match.group("label").strip(), "target": match.group("target").strip(), "hop": "entry"}
...
def compile_reading_chain(entry: str, selection: list[str]) -> ReadingChainPayload:
    ...
    segments: list[ReadingChainSegment] = [{"source": "SKILL.md", "title": _extract_title(skill_body), "content": skill_body.strip()}]
    root_items = _reading_chain(skill_md)
    chosen = next((item for item in root_items if item["key"] == entry), None)
    if chosen is None:
        return {"status": "error", "entry": entry, "error": "entry_not_found", "available_entries": [item["key"] for item in root_items]}
