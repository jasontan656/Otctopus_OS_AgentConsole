...
## Runtime Entry
- Primary runtime entry: `./.venv_backend_skills/bin/python Skills/SkillsManager-Naming-Manager/scripts/Cli_Toolbox.py contract --json`
...
## 2. 必读顺序
1. 先读取 `references/naming_policy.md`。
2. 再读取 `references/registry_policy.md`。
...
- 若需要创建、改造或安装具体技能，本技能只给命名与注册边界，具体骨架仍由 `$SkillsManager-Creation-Template` 与 `$SkillsManager-Mirror-To-Codex` 承接。
