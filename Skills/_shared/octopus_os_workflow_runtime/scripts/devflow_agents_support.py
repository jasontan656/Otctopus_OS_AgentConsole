...
from skill_runtime_context import mother_doc_agents_root, repo_root_from_skill

SKILL_ROOT = mother_doc_agents_root().parents[1]
REPO_ROOT = repo_root_from_skill()
META_DEFAULT_MD_MANAGER_CLI = (
    REPO_ROOT / "Skills" / "Meta-RootFile-Manager" / "scripts" / "Cli_Toolbox.py"
).resolve()
EXTERNAL_AGENTS_TEMPLATE = mother_doc_agents_root() / "EXTERNAL_AGENTS.md"
MACHINE_AGENTS_TEMPLATE = mother_doc_agents_root() / "AGENTS_MACHINE_TEMPLATE.json"
...
def render_external_agents(runtime: dict[str, object], owner: str) -> str:
...
def scaffold_and_collect_devflow_agents(runtime: dict[str, object]) -> dict[str, object]:
    ...
    owner = str(target_contract.get("owner", ""))
    external_agents_path.write_text(render_external_agents(runtime, owner), encoding="utf-8")
