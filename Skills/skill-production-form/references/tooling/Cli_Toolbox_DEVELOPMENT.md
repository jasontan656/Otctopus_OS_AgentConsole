...
- The active local design log must live under `/home/jasontan656/AI_Projects/Codex_Skill_Runtime/skill-production-form`.
- `references/runtime/ITERATION_LOG.md` is now a legacy seed snapshot for first-run migration and bootstrap only.
- The governed result root is `/home/jasontan656/AI_Projects/Codex_Skills_Result/skill-production-form`; future file artifacts must default there unless the caller passes an explicit target path.
