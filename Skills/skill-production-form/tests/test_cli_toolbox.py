...
class TestSkillProductionFormCliTests:
    ...
    def test_working_contract_reads_json_contract(self) -> None:
        ...
        assert payload["payload"]["runtime_observability"]["skill_runtime_root"].endswith("/skill-production-form")
    ...
    def test_default_log_path_uses_runtime_root_and_seeds_legacy_log(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            assert result.returncode == 0, result.stderr
            payload = json.loads(result.stdout)
            expected_log_path = runtime_root / "skill-production-form" / "ITERATION_LOG.md"
            assert payload["log_path"] == str(expected_log_path)
            assert payload["runtime_root"] == str(runtime_root / "skill-production-form")
            assert payload["result_root"] == str(result_root / "skill-production-form")
