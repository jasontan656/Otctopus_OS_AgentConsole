...
```json
{
  ...
  "instruction": [
    ...
    "If the caller provides codex id/session id/resume id, treat it as a context lookup parameter. Read the referenced conversation context first, but do not let that lookup instruction become part of the strengthened prompt.",
  ...
  "workflow": [
    ...
    "If codex/session/resume id is present, fetch the referenced last assistant reply as context before deciding the strengthened intent.",
