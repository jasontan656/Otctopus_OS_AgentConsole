...
- `new-writeback` must fail if external `AGENTS.md` or managed `AGENTS_machine.json` still contains `replace_me`.
- After validation passes, `new-writeback` must re-render `AGENTS_human.md` from external `Part A` plus machine payload and then run lint.
- Source-specific external entry rules remain active during `new-writeback`; for `.codex/skills/AGENTS.md`, `1. 根入口命令` must be a single CLI command that resolves the payload contract, not prose.
