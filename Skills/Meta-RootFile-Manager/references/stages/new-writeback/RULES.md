...
- Managed `AGENTS_machine.json` must not contain `replace_me`.
...
- `.codex/skills/AGENTS.md` requires `1. 根入口命令` to contain only the single `agents-payload-contract` CLI entry for that exact target.
