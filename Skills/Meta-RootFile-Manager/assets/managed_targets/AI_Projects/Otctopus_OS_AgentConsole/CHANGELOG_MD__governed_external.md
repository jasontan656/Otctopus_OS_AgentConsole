---
owner: "由 `Otctopus_OS_AgentConsole` repository root container 所代表的 版本与迭代记录面 负责；当前通过 `$Meta-RootFile-Manager` 的 `CHANGELOG_MD` 通道受管并同步。"
---
# Changelog

All notable public-release changes to Octopus OS should be documented in this file.

## Maintenance Policy

- the public changelog is release-facing
- the private development repository `Otctopus_OS_AgentConsole_AI_dev` remains the high-frequency iteration stream
- release entries should be condensed summaries of the relevant development-period GitHub messages
- the goal is to let the development repository stay fast while the public changelog stays readable

## [Unreleased]

- public release cadence has not started yet
- current work is still focused on internal product shaping, installation boundaries, risk framing, and operator workflow convergence
