...
- 凡是技能真源、技能目录、技能运行时与 mirror/install 主链相关的写入，都先应用这份 repo root 合同。
...
- `SkillsManager-Production-Form` -> `SkillsManager-Naming-Manager` -> `SkillsManager-Creation-Template` -> `SkillsManager-Doc-Structure` -> `SkillsManager-Tooling-CheckUp`
- 若任务已落到 mirror/install，同回合追加 `SkillsManager-Mirror-To-Codex`。
...
2. 协作提醒
- 同级 `README.md` 仍是对外产品摘要入口。
- 安装侧同步沿用既有 mirror/install 主链，不在安装侧直接扩写真源语义。
</reminder>
</part_A>
