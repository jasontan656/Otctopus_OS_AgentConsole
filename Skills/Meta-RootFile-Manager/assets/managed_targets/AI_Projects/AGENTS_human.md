...
```json
{
  ...
  "runtime_constraints": [
    ...
    "before every final reply, hard-execute the Meta-Runtime-Selfcheck turn-end gate instead of treating selfcheck as optional narration"
  ],
  "execution_modes": {
    "READ_EXEC": {
  ...
  "turn_end_actions": [
    ...
    "hard-execute $Meta-Runtime-Selfcheck before final reply",
    "call runtime-contract --json first, then read directive --topic turn-end-selfcheck --json as the default branch",
    "if the run is smooth, skip selfcheck output and continue to the normal final reply",
