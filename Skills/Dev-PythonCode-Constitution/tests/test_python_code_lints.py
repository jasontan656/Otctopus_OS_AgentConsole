...
class TestPythonCodeLintTests:
    ...
    def test_lint_ignores_non_governed_skill_roots_in_aggregate_skills_scan(self) -> None:
        with tempfile.TemporaryDirectory(prefix="py_lint_", dir=str(ROOT.parent)) as tmp:
            root = Path(tmp) / "Skills"
            governed = root / "Meta-RootFile-Manager" / "scripts"
            ignored_system = root / ".system" / "Skill-creator" / "scripts"
            ignored_creation = root / "SkillsManager-Creation-Template" / "scripts"
