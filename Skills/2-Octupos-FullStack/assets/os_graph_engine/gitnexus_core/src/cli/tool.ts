...
async function getBackend(): Promise<LocalBackend> {
  ...
  if (!ok) {
    console.error('OS_graph: No indexed repositories found. Run: os-graph-engine-core analyze');
    process.exit(1);
  }
  return _backend;
}
...
export async function renameCommand(options?: {
  ...
  if (!options?.newName?.trim()) {
    console.error('Usage: os-graph-engine-core rename --new-name <name> [--symbol-name <name>|--symbol-uid <uid>]');
    process.exit(1);
  }
...
export async function resourceCommand(uri: string): Promise<void> {
  if (!uri?.trim()) {
    console.error('Usage: os-graph-engine-core resource <uri>');
    process.exit(1);
  }
