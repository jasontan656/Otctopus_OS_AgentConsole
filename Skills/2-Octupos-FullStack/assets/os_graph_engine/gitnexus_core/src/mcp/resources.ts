...
async function getReposResource(backend: LocalBackend): Promise<string> {
  ...
  if (repos.length === 0) {
    return 'repos: []\n# No repositories indexed. Run: os-graph-engine-core analyze';
  }
  ...
  if (repos.length > 1) {
    ...
    lines.push(`# os-graph-engine-core query "auth" --repo "${repos[0].name}"`);
  }
...
async function getContextResource(backend: LocalBackend, repoName?: string): Promise<string> {
  ...
  if (!context) {
    return 'error: No codebase loaded. Run: os-graph-engine-core analyze';
  }
  ...
  lines.push('');
  lines.push('re_index: Run `os-graph-engine-core analyze` in terminal if data is stale');
  lines.push('');
...
async function getClustersResource(backend: LocalBackend, repoName?: string): Promise<string> {
  try {
    ...
    if (!result.clusters || result.clusters.length === 0) {
      return 'modules: []\n# No functional areas detected. Run: os-graph-engine-core analyze';
    }
    ...
    if (result.clusters.length > displayLimit) {
      lines.push(`\n# Showing top ${displayLimit} of ${result.clusters.length} modules. Use os-graph-engine-core query for deeper search.`);
    }
...
async function getProcessesResource(backend: LocalBackend, repoName?: string): Promise<string> {
  try {
    ...
    if (!result.processes || result.processes.length === 0) {
      return 'processes: []\n# No processes detected. Run: os-graph-engine-core analyze';
    }
    ...
    if (result.processes.length > displayLimit) {
      lines.push(`\n# Showing top ${displayLimit} of ${result.processes.length} processes. Use os-graph-engine-core query for deeper search.`);
    }
