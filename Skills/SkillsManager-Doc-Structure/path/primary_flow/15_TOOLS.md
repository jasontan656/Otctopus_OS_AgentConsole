...
- `python3 ./scripts/Cli_Toolbox.py read-contract-context --entry <entry_key> --json`
...
- `read-contract-context`：当前技能自身的快捷合同编译入口；直接读取本技能某个功能入口的完整链路。
- `read-path-context`：等价别名，返回同一编译结果。
