---
doc_id: tooling.category.index
doc_type: tooling_index
topic: Category index for document-structure runtime and tooling docs
anchors:
- target: 00_ARCHITECTURE_OVERVIEW.md
  relation: indexes
  direction: downstream
  reason: Architecture overview is the top-level entry for tooling docs.
- target: modules/mod_docstructure_core.md
  relation: indexes
  direction: downstream
  reason: The core module doc is a primary tooling branch.
---

# Category Index

## 文档分层
- runtime：
  - `../../runtime/SKILL_DOCSTRUCTURE_RUNTIME_OVERVIEW.md`
- tooling：
  - `00_ARCHITECTURE_OVERVIEW.md`
  - `modules/mod_docstructure_core.md`
