...
- 若任务涉及 typing、data boundary、concurrency、exception、logging、subprocess、HTTP timeout 或 Python 运行时安全边界，应同步读取 `../python_rules/PYTHON_RUNTIME_SAFETY_CONSTITUTION.md`。
- 当前 lint 还负责七类 Python 工程安全规则：公共函数类型注解边界、payload/data contract 边界、并发生命周期边界、异常治理边界、`subprocess` 调用安全边界、HTTP timeout 边界、`logging` 命名 logger 边界。
