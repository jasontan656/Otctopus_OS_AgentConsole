...
- 本技能自身一旦写回，必须先经过 keyword-first 裁决，不允许继续叠 compatibility/legacy 补丁。
</part_A>
...
```json
{
  ...
  "instruction": [
    ...
    "Before any Meta-Runtime-Selfcheck file writeback, run the keyword-first-edit-governance decision so the fix lands as rewrite > replace > add instead of patch stacking.",
  ...
  "workflow": [
    ...
    "Read the whole governed object or the smallest semantically complete unit before deciding how to edit it.",
    "Apply rewrite/delete first, direct keyword-first replacement second, and additive completion only as the final option.",
    "If rewrite/delete/whole-block replacement is required, enumerate the exact deletion scope and request user confirmation before removing anything.",
    "Apply the chosen governed repair, then run the canonical verification command or the nearest governed inspection step.",
  ...
  "rules": [
    ...
    "Do not add compatibility layers, legacy shells, mappings, aliases, or dual-track bridges when rewrite or direct replacement is sufficient."
  ]
}
```
</part_B>
