...
- Load working contract:
  - `cd /home/jasontan656/AI_Projects/octopus-os-agent-console/Skills/production-form && python3 scripts/Cli_Toolbox.py working-contract --json | sed -n '1,200p'`
- Load current intent snapshot:
  - `cd /home/jasontan656/AI_Projects/octopus-os-agent-console/Skills/production-form && python3 scripts/Cli_Toolbox.py intent-snapshot --json | sed -n '1,200p'`
- Load latest log:
  - `cd /home/jasontan656/AI_Projects/octopus-os-agent-console/Skills/production-form && python3 scripts/Cli_Toolbox.py latest-log --json | sed -n '1,200p'`
- Append a new local design log entry:
  - `cd /home/jasontan656/AI_Projects/octopus-os-agent-console/Skills/production-form && python3 scripts/Cli_Toolbox.py append-iteration-log --title "Refine installer boundary" --summary "Locked the temporary install boundary between product surfaces and codex skill roots." --decision "Keep product docs outside ~/.codex/skills" --affected-path "product_tools/octopus_os_agent_console.py" --next-step "Fold the new rule into the public install flow" --json | sed -n '1,200p'`
