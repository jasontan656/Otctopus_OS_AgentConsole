...
- 目标项目的开发文档承载总容器固定为 `<target_root>/Development_Docs/`，其下一级目录固定为模块容器；本技能只在模块容器内工作。
- 目标项目根还必须位于 `AI_Projects` workspace 内；否则 `$Meta-Default-md-manager` 无法收治目标模块 `AGENTS.md`，本技能必须拒绝服务。
- 目标项目的 `Development_Docs/` 与目标模块子目录必须先存在；任一不存在时，本技能必须拒绝服务。
- 开发文档的落盘容器必须先由 `Dev-OctopusOS-Constitution-ProjectStructure` 判定；若目标项目尚未固定其他特殊容器，则模块文档根固定为 `<target_root>/Development_Docs/<module_dir>/`。
...
- 目标模块的外部 `AGENTS.md` 必须由本技能模版创建，并由 `$Meta-Default-md-manager` 收治；创建后必须立即执行 `collect`，使外部 `AGENTS.md` 与技能内治理映射形成闭环。
- 详细规则必须从下方入口文档读取，不得只凭本门面自行发挥。
- 默认路径解析：
  - `target_root`：当前目标项目根；默认取当前工作目录，必要时显式传入
  - `development_docs_root`：固定为 `<target_root>/Development_Docs`
  - `module_dir`：`Development_Docs/` 下的目标模块目录名；必须显式给定或由已固定容器推导
  - `docs_root`：当前模块文档根，固定为 `<target_root>/Development_Docs/<module_dir>`
  - `mother_doc_root`：`<docs_root>/mother_doc`
  - `construction_packs_root`：`<mother_doc_root>/execution_atom_plan_validation_packs`
  - `codebase_root`：默认 `<target_root>`
...
2. 进入任一阶段前，必须先读取目标位置预检：
   - `python3 scripts/Cli_Toolbox.py target-runtime-contract --target-root <target_root> --module-dir <module_dir> [--docs-root <docs_root>] [--codebase-root <codebase_root>] [--graph-runtime-root <graph_runtime_root>] --json`
3. 进入任一阶段前，必须先读取：
   - `python3 scripts/Cli_Toolbox.py stage-checklist --stage <mother_doc|construction_plan|implementation|acceptance> --target-root <target_root> --module-dir <module_dir> --json`
4. 当前阶段的读物边界只从工具取：
...
- 外部辅助层：
  ...
  - `/home/jasontan656/AI_Projects/octopus-os-agent-console/Skills/Meta-Default-md-manager/SKILL.md`
...
- `$Meta-Default-md-manager` 负责把目标模块 `AGENTS.md` 收治进治理映射模版；本技能必须通过它完成 `AGENTS` 闭环，而不是自行绕开治理链。
