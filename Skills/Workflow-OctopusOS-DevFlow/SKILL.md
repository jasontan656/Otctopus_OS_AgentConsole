...
- 本技能不再绑定固定单项目目录；它按 `target_root + development_docs_root + docs_root + codebase_root + graph_runtime_root` 解析当前任务载体。
- `target_root` 在本技能内表示 `AI_Projects` 内的 repo/workspace 根边界，不再默认等同代码对象根。
- 当前开发文档承载总容器由 `development_docs_root` 显式指定或推导，其下一级目录固定为当前开发主题容器；本技能只在该主题容器内工作。
- 目标项目根还必须位于 `AI_Projects` workspace 内；否则 `$Meta-RootFile-Manager` 无法收治目标模块 `AGENTS.md`，本技能必须拒绝服务。
- 目标代码对象的 `Development_Docs/` 与当前工作主题子目录必须先存在；任一不存在时，本技能必须拒绝服务。
- 仅在项目结构初始化阶段，`Development_Docs/` 容器允许为空；一旦本技能介入具体开发闭环，才要求目标工作主题子目录已经存在。
- 开发文档的落盘容器必须先由 `Dev-OctopusOS-Constitution-ProjectStructure` 判定；若目标项目尚未固定其他特殊容器，则默认容器是 `<codebase_root>/Development_Docs/`，模块文档根固定为 `<development_docs_root>/<module_dir>/`，其中 `<module_dir>` 表示当前开发主题 slug，不表示再次重复对象名。
...
- 默认路径解析：
  ...
  - `development_docs_root`：当前代码对象自己的 `Development_Docs` 容器；优先显式传入，默认回退到 `<target_root>/Development_Docs`
  - `module_dir`：`development_docs_root` 下的当前开发主题目录名；必须显式给定或由 `docs_root` 推导；它不是对象名的重复一层
  - `docs_root`：当前开发主题文档根，固定为 `<development_docs_root>/<module_dir>`
  - `mother_doc_root`：`<docs_root>/mother_doc`
  - `construction_packs_root`：`<mother_doc_root>/execution_atom_plan_validation_packs`
  - `codebase_root`：默认 `<development_docs_root>/..`
...
2. 进入任一阶段前，必须先读取目标位置预检：
   - `./.venv_backend_skills/bin/python Skills/Workflow-OctopusOS-DevFlow/scripts/Cli_Toolbox.py target-runtime-contract --target-root <target_root> --development-docs-root <development_docs_root> --module-dir <module_dir> [--docs-root <docs_root>] [--codebase-root <codebase_root>] [--graph-runtime-root <graph_runtime_root>] --json`
3. 进入任一阶段前，必须先读取：
   - `./.venv_backend_skills/bin/python Skills/Workflow-OctopusOS-DevFlow/scripts/Cli_Toolbox.py stage-checklist --stage <mother_doc|construction_plan|implementation|acceptance> --target-root <target_root> --development-docs-root <development_docs_root> --module-dir <module_dir> --json`
4. 当前阶段的读物边界只从工具取：
