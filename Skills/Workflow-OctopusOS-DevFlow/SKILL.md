...
2. 进入任一阶段前，必须先读取目标位置预检：
   - `./.venv_backend_skills/bin/python Skills/Workflow-OctopusOS-DevFlow/scripts/Cli_Toolbox.py target-runtime-contract --target-root <target_root> --module-dir <module_dir> [--docs-root <docs_root>] [--codebase-root <codebase_root>] [--graph-runtime-root <graph_runtime_root>] --json`
3. 进入任一阶段前，必须先读取：
   - `./.venv_backend_skills/bin/python Skills/Workflow-OctopusOS-DevFlow/scripts/Cli_Toolbox.py stage-checklist --stage <mother_doc|construction_plan|implementation|acceptance> --target-root <target_root> --module-dir <module_dir> --json`
4. 当前阶段的读物边界只从工具取：
