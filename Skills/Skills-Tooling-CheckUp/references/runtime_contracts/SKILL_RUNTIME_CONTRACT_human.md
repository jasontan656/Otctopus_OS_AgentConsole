...
<part_A>
- 人类阅读入口：本文件说明 `Skills-Tooling-CheckUp` 已切换为 CLI-first runtime contract。
- 模型运行时入口固定为：
...
```json
{
  ...
  "skill_name": "Skills-Tooling-CheckUp",
  ...
  "target_skill_handoff": [
    ...
    "If Python files are edited, run Dev-PythonCode-Constitution-Backend lint on the concrete Python target scope before closing the turn."
  ]
}
```
</part_B>
