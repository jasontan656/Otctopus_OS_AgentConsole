...
- 当前已经内置第八个稳定子域：临时文件集中治理区，用于存放临时需要治理的 markdown 文件或任意其他文件。
...
- 当任务涉及临时需要治理的 markdown、临时证据文件、一次性整理稿或其他尚未决定长期归宿的文件时，必须进入临时文件分支，落到集中区并维护 README。
...
- 适用于：把临时需要治理的 markdown 文件、整理中的零散文件、临时证据附件或其他短期待处理文件迁入临时文件区，并按当时意图决定名字或子目录。
