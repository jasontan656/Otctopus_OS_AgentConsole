...
## 在项目中起什么作用
- 约束 Telegram 相关输出必须覆盖渠道、前端、后端、数据库与通用治理五域并集。
- 防止只给 Bot 代码片段而缺失 WebApp、回执链路、会话隔离等关键要素。
...
| 触发关键词域 | 必扩展域 |
|---|---|
| `telegram/bot` | `telegram_channel_domain + frontend_domain + backend_domain + database_domain + common_always_on` |
| `webhook/callbackquery/inline_button` | 同上五域并集 |
| `deep_link/sendDocument/webapp` | 同上五域并集 |

## 写法风格（命令式）
- 先判定 Telegram 触发，再挂五域并集，再校验主题完整性。
- 输出必须写链路，不允许单点实现建议。
- 缺域立即阻断并返回缺失项。
...
| `required_domains` | 必须命中五域并集 |
| `required_topics` | 必含回执策略、重试策略、消息映射、会话绑定、权限边界 |
| `webapp_rule` | WebApp 场景必须包含路由与 session 绑定策略 |
...
## 必须做（Do）
1. 命中 Telegram 关键词时强制挂载五域并集。
2. 输出必须写消息回执、失败重试、映射规则。
3. WebApp 场景必须写路由绑定和会话隔离策略。
4. 缺域或缺主题时立即阻断并返回结构化缺失项。
5. 发布前执行 Telegram 扩展完整性检查，失败阻断。

## 不要做（Don't）
1. 不要只给 Bot API 调用代码。
2. 不要忽略 WebApp 与后端协议一致性。
3. 不要忽略权限边界和审计要求。
4. 不要省略失败回执与重试策略。
...
2. 缺任一域会导致交互断链或不可审计。
3. 五域并集是把“能回复消息”升级为“可运营系统”的最低条件。
4. 强阻断机制可把漏项前移到设计阶段。
...
# 3) 五域并集检查
rg -n "telegram_channel_domain|frontend_domain|backend_domain|database_domain|common_always_on" references/knowledge_graph/MOL_TECH_STACK_KEYWORD_ANCHOR_GRAPH_v1.md
...
2. WebApp 场景路由+session 绑定策略覆盖率达到 100%。
3. 缺域阻断命中率为 100%，无漏放行。
