...
export function getResourceTemplates(): ResourceTemplate[] {
  return [
    {
      uriTemplate: 'codegraph://repo/{name}/context',
      name: 'Repo Overview',
      description: 'Codebase stats, staleness check, and available tools',
      mimeType: 'text/yaml',
    },
    {
      uriTemplate: 'codegraph://repo/{name}/clusters',
      name: 'Repo Modules',
      description: 'All functional areas (Leiden clusters)',
      mimeType: 'text/yaml',
    },
    {
      uriTemplate: 'codegraph://repo/{name}/processes',
      name: 'Repo Processes',
      description: 'All execution flows',
      mimeType: 'text/yaml',
    },
    {
      uriTemplate: 'codegraph://repo/{name}/schema',
      name: 'Graph Schema',
      description: 'Node/edge schema for Cypher queries',
      mimeType: 'text/yaml',
    },
    {
      uriTemplate: 'codegraph://repo/{name}/cluster/{clusterName}',
      name: 'Module Detail',
      description: 'Deep dive into a specific functional area',
      mimeType: 'text/yaml',
    },
    {
      uriTemplate: 'codegraph://repo/{name}/process/{processName}',
      name: 'Process Trace',
...
function parseUri(uri: string): { repoName?: string; resourceType: string; param?: string } {
  if (uri === 'codegraph://repos') return { resourceType: 'repos' };

  // Repo-scoped: codegraph://repo/{name}/context
  const repoMatch = uri.match(/^codegraph:\/\/repo\/([^/]+)\/(.+)$/);
  if (repoMatch) {
...
async function getReposResource(backend: LocalBackend): Promise<string> {
  ...
  if (repos.length === 0) {
    return 'repos: []\n# No repositories indexed. Run: meta-code-graph-base-core analyze';
  }
  ...
  if (repos.length > 1) {
    ...
    lines.push(`# meta-code-graph-base-core query "auth" --repo "${repos[0].name}"`);
  }
...
async function getContextResource(backend: LocalBackend, repoName?: string): Promise<string> {
  ...
  if (!context) {
    return 'error: No codebase loaded. Run: meta-code-graph-base-core analyze';
  }
  ...
  lines.push('');
  lines.push('re_index: Run `meta-code-graph-base-core analyze` in terminal if data is stale');
  lines.push('');
  lines.push('resources_available:');
  lines.push('  - codegraph://repos: All indexed repositories');
  lines.push(`  - codegraph://repo/${context.projectName}/clusters: All functional areas`);
  lines.push(`  - codegraph://repo/${context.projectName}/processes: All execution flows`);
  lines.push(`  - codegraph://repo/${context.projectName}/cluster/{name}: Module details`);
  lines.push(`  - codegraph://repo/${context.projectName}/process/{name}: Process trace`);
...
async function getClustersResource(backend: LocalBackend, repoName?: string): Promise<string> {
  try {
    ...
    if (!result.clusters || result.clusters.length === 0) {
      return 'modules: []\n# No functional areas detected. Run: meta-code-graph-base-core analyze';
    }
    ...
    if (result.clusters.length > displayLimit) {
      lines.push(`\n# Showing top ${displayLimit} of ${result.clusters.length} modules. Use meta-code-graph-base-core query for deeper search.`);
    }
...
async function getProcessesResource(backend: LocalBackend, repoName?: string): Promise<string> {
  try {
    ...
    if (!result.processes || result.processes.length === 0) {
      return 'processes: []\n# No processes detected. Run: meta-code-graph-base-core analyze';
    }
    ...
    if (result.processes.length > displayLimit) {
      lines.push(`\n# Showing top ${displayLimit} of ${result.processes.length} processes. Use meta-code-graph-base-core query for deeper search.`);
    }
...
function getSchemaResource(): string {
  return `# Meta-code-graph-base Schema
