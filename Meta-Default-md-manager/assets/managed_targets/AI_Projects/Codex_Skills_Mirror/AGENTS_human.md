...
2. 文档语义标准
- 单文档单主题。
- 单主题必须同时满足：单职责、单边界、单消费者、单运行层。
- 文档正文只允许解释本主题自身的规则、结构、约束、输入输出。
- 如果必须提到相邻主题，只能做“引用式声明”，不能在当前文档内展开对方语义。
- 发现“动作语义”和“对象语义”混写时，必须拆分。
- 发现“内容结构语义”和“运行阶段语义”混写时，必须拆分。
- 发现“治理规则”和“工具定义”混写时，必须拆分。
